...
# 簡易的なONNXモデル作成用のユーティリティ（デモ用）
class DummyONNXAnalyzer:
    """ONNX環境が利用できない場合のダミー実装"""
    
    def __init__(self, *args, **kwargs):
        logger.warning("ダミーONNX分析器を使用しています")
    ...
    def analyze(self, text: str) -> Tuple[float, SentimentCategory, Dict[str, float]]:
        ...
        # 簡単なルールベースのフォールバック
        if any(word in text for word in ['嬉しい', '楽しい', '最高', '素晴らしい']):
            score = random.uniform(70, 90)
        elif any(word in text for word in ['悲しい', '辛い', '嫌', '困った']):
        ...
        if score > 60:
            ...
            probs = {'positive': 0.3, 'neutral': 0.4, 'negative': 0.3}
        
        return score, category, probs
    ...
    def get_model_info(self) -> Dict[str, any]:
        return {
            'available': True,
            'type': 'dummy',
            'note': 'This is a dummy implementation for testing purposes'
        } 