...
export const CategoryNavigator: React.FC = () => {
  ...
  const handleCategoryClick = (category: Category) => {
    // 次の階層が存在するかチェック
    if (selectedPath.length === 0 && category.id && subCategories[category.id]) {
      // メインカテゴリー→サブカテゴリーへ
      setSelectedPath([category.id]);
    } else if (selectedPath.length === 1 && category.id && subSubCategories[category.id]) {
      // サブカテゴリー→サブサブカテゴリーへ
  ...
  const title =
    selectedPath.length === 0
      ...
      ? mainCategories.find((c) => c.id === selectedPath[0])?.title || "詳細カテゴリー"
      : subCategories[selectedPath[0]]?.find((c) => c.id === selectedPath[1])?.title || "詳細カテゴリー";

  return (
    <div className="max-w-2xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        <motion.h2 className="text-xl font-semibold" layout initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          {title}
        </motion.h2>
        {selectedPath.length > 0 && (
      ...
      {/* カテゴリーグリッド */}
      <CategoryGrid categories={displayedCategories} onCategoryClick={handleCategoryClick} />
    </div>
  );
};
